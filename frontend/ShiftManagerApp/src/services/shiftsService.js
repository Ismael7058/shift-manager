...
export const ShiftService = {
  ...
  getShifts: (providerId, clientId, filter) =>
    apiFetch(`/shifts?providerId=${providerId}&clientId=${clientId}&ServiceId=${filter.serviceId}&CreatedById=${filter.createdById}&CanceledById=${filter.canceledById}&DateFrom=${filter.dateFrom}&DateTo=${filter.dateTo}&MinPrice=${filter.minPrice}&MaxPRice=${filter.maxPrice}${filter.statuses && filter.statuses.length > 0 ? `&Statuses=${filter.statuses}` : ''}&ProviderName=${filter.providerName}&ClientName=${filter.clientName}&SortBy=${filter.sortBy}&IsDescending=${filter.isDescending}&PageNumber=${filter.pageNumber}&$PageSize=${filter.pageSize}`),
